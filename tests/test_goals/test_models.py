...
def test_image_model_for_goal():
    image = ImageModel(
        goal_id=1,
        img_url="/static/images/goals/book_cover.jpg"
    )
...
def test_image_model_for_event():
    image = ImageModel(
        event_id=1,
        img_url="/static/images/events/running.jpg"
    )
...
def test_image_model_with_both_ids_none():
    image = ImageModel(
        img_url="/static/images/general.jpg"
    )
...
def test_goal_completion_logic():
    ...
    for i in range(3):
        event = ProgressEventModel(
            goal_id=goal.id if goal.id else 1,
            value=1,
            description=f"Read book {i + 1}"
        )
        events.append(event)
