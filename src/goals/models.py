...
class GoalModel(Base):
    ...
    end_date: Mapped[date | None] = mapped_column(
        Date, nullable=True
    )  # дата завершения, выбирает пользователь
    ...
    # Связи
    parent: Mapped["GoalModel | None"] = relationship(
        "GoalModel", remote_side=[id], back_populates="children"
    )
    children: Mapped[list["GoalModel"]] = relationship(
        "GoalModel", back_populates="parent", cascade="all, delete-orphan"
    )
    events: Mapped[list["ProgressEventModel"]] = relationship(
        "ProgressEventModel", back_populates="goal"
    )
    images: Mapped[list["ImageModel"]] = relationship(
        "ImageModel", back_populates="goal"
    )
...
class ImageModel(Base):
    ...
    # Связи
    goal: Mapped["GoalModel | None"] = relationship(
        "GoalModel", back_populates="images"
    )
    event: Mapped["ProgressEventModel | None"] = relationship(
        "ProgressEventModel", back_populates="images"
    )
