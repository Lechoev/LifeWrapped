        ...
        async with self.uow_factory() as uow:
            ...
            if parent_id:
                ...
                if not parent_exists:
                    logger.warning("Parent goal not found", extra={"user_id": user_id, "parent_id": parent_id})
                    raise ParentNotFoundError(f"Родительская цель {parent_id} не найдена")

            result = await uow.goals.create_goal(data={**goal_data, "user_id": user_id})
            logger.info("Goal created", extra={"user_id": user_id, "goal_id": result.id})
            return result

    async def get_all_goals(self, user_id: int):
        async with self.uow_factory() as uow:
            result = await uow.goals.get_all_by_user(user_id)
            logger.info("Fetched all goals", extra={"user_id": user_id, "count": len(result)})
            return result
        ...
        async with self.uow_factory() as uow:
            ...
            if new_parent_id is not None:
                if new_parent_id == goal_id:
                    logger.warning(
                        "Goal cannot be parent of itself", extra={"user_id": user_id, "goal_id": goal_id}
                    )
                ...
                if not parent_exists:
                    logger.warning("Parent goal not found for update",
                                   extra={"user_id": user_id, "goal_id": goal_id, "parent_id": new_parent_id})
