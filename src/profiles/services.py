        ...
        async with self.uow_factory() as uow:
            ...
            if existing_profile:
                raise exceptions.ProfileAlreadyExistsError(
                    f"Профиль для пользователя {user_id} уже существует"
                )

            await uow.profiles.create_profile(data={**profile_data, "user_id": user_id})
        ...
        async with self.uow_factory() as uow:
            ...
            if not profile:
                raise exceptions.ProfileNotFoundError(
                    f"Профиль для пользователя {user_id} не найден"
                )
